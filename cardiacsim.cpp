...
void print_2D(double **myArray,int height, int width){
...
// serial simulate
void simulate_version0 (double** E,  double** E_prev,double** R,
...
int main (int argc, char** argv)
{
  ...
  //Host to device Mem copy
  cout << "Sizes on Host Device, E  : " << sizeof(E) << endl; 
  cout << "Sizes on Host Device, E_prev  : " << sizeof(E_prev) << endl; 
  cout << "Sizes on Host Device, R  : " << sizeof(R) << endl; 
  ...
  // Cuda Allocations
  double* d_E = NULL;
  double* d_E_prev = NULL;
  double* d_R = NULL;

  int size = (m+2) * (n+2);
  unsigned int alloc_space = sizeof(double)*size;

  if (kernel > 0){
    cudaMalloc((void**)&d_E, alloc_space);
    cudaMalloc((void**)&d_E_prev, alloc_space);
    cudaMalloc((void**)&d_R, alloc_space);

    cudaMemcpy(d_E,E[0],alloc_space,cudaMemcpyHostToDevice);
    cudaMemcpy(d_E_prev,E_prev[0],alloc_space,cudaMemcpyHostToDevice);
    cudaMemcpy(d_R,R[0],alloc_space,cudaMemcpyHostToDevice);
  } // if (kernel > 0)
  ...
  //tmp variable for exchanging
  double **tmp;

 while (t<T) {
  // Do it inside the .cu file
    ...
    switch (kernel){
    case 0:
      simulate_version0(E, E_prev, R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b); 
      tmp = E; 
      E = E_prev; 
      E_prev = tmp;
      break;
    ...
    case 1:
      simulate_version1(d_E, d_E_prev, d_R, bx, by, alpha, n, m, kk, dt, a, epsilon, M1, M2, b);
      break;

    case 2:
      simulate_version2(d_E, d_E_prev, d_R, bx, by, alpha, n, m, kk, dt, a, epsilon, M1, M2, b);
      break;

    case 3:
      simulate_version3(d_E, d_E_prev, d_R, bx, by, alpha, n, m, kk, dt, a, epsilon, M1, M2, b);
      break;

    case 4:
      simulate_version4(d_E, d_E_prev, d_R, bx, by, alpha, n, m, kk, dt, a, epsilon, M1, M2, b);
      break;
    ...
    if (plot_freq){
        if(kernel > 0){
          cudaMemcpy(E[0],d_E,alloc_space,cudaMemcpyDeviceToHost);
        }
  ...
  if(kernel > 0){
    cudaMemcpy(E[0],d_E,alloc_space,cudaMemcpyDeviceToHost);
    cudaMemcpy(E_prev[0],d_E_prev,alloc_space,cudaMemcpyDeviceToHost);
    cudaMemcpy(R[0],d_R,alloc_space,cudaMemcpyDeviceToHost);
  }
